...
class AccountMove(models.Model):
    ...
    system_invoice_no = fields.Char('System Invoice No')

    def print_einvoice(self):
        return self.env.ref('nascon_invoice.nascon_invoice_report').sudo().report_action(self)
    ...
    def _ubl_add_attachments(self, parent_node, ns, version="2.1"):
        ...
        pdf_inv = self.with_context(ctx).env.ref(
            'nascon_invoice.nascon_invoice_report')._render_qweb_pdf(self.ids)[0]
    ...
    def _get_invoice_report_names(self):
        return [
            ...
            "nascon_invoice.invoice_format_view3",
